...
import com.taro.utils.SerializationSessionUtils;
...
public class RedisSessionDao extends CachingSessionDAO {
    ...
    //30分钟,单位是秒
    private int timeToLiveSeconds = 10800;
	...
	protected Serializable doCreate(Session session) {
		...
		String key = SerializationSessionUtils.sessionKey(keyPrefix, session);
		String value = SerializationSessionUtils.sessionFromString(session);
		
		template.setex(key, timeToLiveSeconds, value);	//插入数据存储，并且在指定时间后过期
		return sessionId;
	}
	...
	protected Session doReadSession(Serializable sessionId) {
		...
		String value = (String) template.get(SerializationSessionUtils.sessionKey(keyPrefix, sessionId));
		//例如 Redis 调用 flushdb 情况下所有的数据，读到的session就是空的
    	if(value != null){
    		Session session = (Session) SerializationSessionUtils.sessionIdFromString(value);
	...
	protected void doUpdate(Session session) {
        ...
        String key = SerializationSessionUtils.sessionKey(keyPrefix, session);
        String value = SerializationSessionUtils.sessionFromString(session);
        template.setex(key,timeToLiveSeconds,value);
	}

	@Override
	protected void doDelete(Session session) {
		logger.debug("=> Delete session with ID [{}]", session.getId());
		template.del(SerializationSessionUtils.sessionKey(keyPrefix, session));
